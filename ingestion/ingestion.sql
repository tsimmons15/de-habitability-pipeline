...
create table tsimmons.metadata (
	job_id SERIAL primary key,
	last_updated TIMESTAMPTZ NOT null default now(),
	status VARCHAR(20) NOT NULL CHECK (status IN ('Running', 'Succeeded', 'Failed'))
);
create index i_last_updated on tsimmons.metadata (last_updated);

create table tsimmons.usgs_silver (
	mag FLOAT,
	place VARCHAR(50),
	time TIMESTAMPTZ,
	updated TIMESTAMPTZ,
...
create table tsimmons.usgs_silver_insert (
	mag FLOAT,
	place VARCHAR(50),
	time TIMESTAMPTZ,
	updated TIMESTAMPTZ,
...
select * from tsimmons.usgs_insert;
...
create table tsimmons.census_silver (
	name_key VARCHAR(50),
	name VARCHAR(50),
	pop INT,
	update_tm TIMESTAMPTZ default now(),
...
create table tsimmons.census_silver_insert (
	name VARCHAR(50),
	pop INT,
	primary key (name_key)
);
...
create table tsimmons.geocode_silver (
	name_key VARCHAR(50),
	name VARCHAR(50),
	lat FLOAT,
	lon FLOAT,
	...
	state VARCHAR(50),
	update_tm TIMESTAMPTZ default now(),
	primary key (name_key, lat, lon)
);
create table tsimmons.geocode_silver_insert (
	name_key VARCHAR(50),
	name VARCHAR(50),
	lat FLOAT,
	lon FLOAT,
...
create table tsimmons.weather_silver (
	lat FLOAT,
	lon FLOAT,
	tz VARCHAR(25),
	date TIMESTAMPTZ,
...
create table tsimmons.weather_silver_insert (
	lat FLOAT,
	lon FLOAT,
	tz VARCHAR(25),
	date TIMESTAMPTZ,
...
CREATE OR REPLACE PROCEDURE truncate_silver_insert_tables()
LANGUAGE plpgsql
AS $$
BEGIN
	SET CONSTRAINTS ALL IMMEDIATE;
    truncate tsimmons.usgs__silver_insert;
	truncate tsimmons.geocode_silver_insert;
	truncate tsimmons.census_silver_insert;
	truncate tsimmons.weather_silver_insert;
EXCEPTION WHEN OTHERS THEN
	RAISE;
END;
$$;
...
CREATE OR REPLACE PROCEDURE merge_silver_insert_raw()
LANGUAGE plpgsql
AS $$
BEGIN
	SET CONSTRAINTS ALL IMMEDIATE;

	select max(last_updated) into update_time from tsimmons.metadata;

	MERGE INTO tsimmons.weather_silver AS raw
	USING tsimmons.weather_silver_insert AS src
	ON raw.lat = src.lat and raw.lon = src.lon and raw.date = src.date and raw.last_updated > update_time
	WHEN MATCHED THEN
    	UPDATE SET lat = src.lat, lon = src.lon, tz = src.tz,
							date = src.date, units = src.units, cloud_cover = src.cloud_cover,
							humidity = src.humidity, precipitation = src.precipitation, 
	...
	MERGE INTO tsimmons.geocode_silver AS raw
	USING tsimmons.geocode_silver_insert AS src
	ON raw.lat = src.lat and raw.lon = src.lon and raw.last_updated > update_time
	WHEN MATCHED THEN
    	UPDATE SET name = src.name, lat = src.lat, lon = src.lon, country = src.country, state = src.state
	WHEN NOT MATCHED THEN
    	INSERT (name, lat, lon, country, state) VALUES (src.name, src.lat, src.lon, src.country, src.state);

	MERGE INTO tsimmons.census_silver AS raw
	USING tsimmons.census_silver_insert AS src
	ON raw.name = src.name and raw.last_updated > update_time
	WHEN MATCHED THEN
    	UPDATE SET name = src.name, pop = src.pop
	WHEN NOT MATCHED THEN
    	INSERT (name, pop) VALUES (src.name, src.pop);

	MERGE INTO tsimmons.usgs_silver AS raw
	USING tsimmons.usgs_silver_insert AS src
	ON raw.place = src.place and raw.time = src.time and raw.last_updated > update_time
	WHEN MATCHED THEN
    	UPDATE SET mag = src.mag, place = src.place, time = src.time, updated = src.updated, tz = src.tz, 
					felt = src.felt, cdi = src.cdi, mmi = src.mmi, alert = src.alert, status = src.status, 
					tsunami = src.tsunami, sig = src.sig, net = src.net, code = src.code, nst = src.nst, 
